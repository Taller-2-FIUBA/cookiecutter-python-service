...
@patch.dict(environ, {"{{ cookiecutter.project_slug|upper }}_PROMETHEUS_PORT": "9004"}, clear=True)
def test_when_environment_has_prometheus_port_9004_expect_9004():
    cnf = to_config(AppConfig)
    assert cnf.prometheus_port == 9004


@patch.dict(environ, {"{{ cookiecutter.project_slug|upper }}_LOG_LEVEL": "DEBUG"}, clear=True)
